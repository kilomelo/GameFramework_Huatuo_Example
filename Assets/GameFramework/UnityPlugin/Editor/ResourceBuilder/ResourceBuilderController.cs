...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        public long DynamicAssemblyPlatforms
        {
            get;
            set;
        }
        ...
        public bool Load()
        {
            ...
            DynamicAssemblyPlatforms = GetHuatuoSetting();
            ApplyHuatuoSymbols(DynamicAssemblyPlatforms);
        ...
        public bool Save()
        {
            long preHuatuoConfigValue = 0;
            preHuatuoConfigValue = GetHuatuoSetting();
            SaveHuatuoSettings();
            try
            {
                XmlDocument xmlDocument = new XmlDocument();
                ...
                if (preHuatuoConfigValue != DynamicAssemblyPlatforms)
                    ApplyHuatuoSymbols(DynamicAssemblyPlatforms);
        ...
        private long GetHuatuoSetting()
        {
            if (File.Exists(HuatuoDefine.EditorConfigPath))
            {
                // Debug.Log($"Huatuo config file [{HuatuoDefine.EditorConfigPath}] found.");
                using (TOMLParser parser = new TOMLParser(File.OpenText(HuatuoDefine.EditorConfigPath)))
                {
                    IEnumerable<TomlSyntaxException> errors;
                    if (parser.TryParse(out TomlNode rootNode, out errors))
                    {
                        var node = rootNode.FindNode("UnityGameFramework.HuatuoSettings.DynamicAssemblyPlatforms");
                        // UnityEngine.Debug.Log($"UnityGameFramework.HuatuoSettings.DynamicAssemblyPlatforms: [ {node} ]");
                        if (null != node && null != node.AsInteger) return node.AsInteger.Value;
                        return 0;
                    }
                }
            }
            ...
            {
                Debug.LogWarning($"Huatuo config file [{HuatuoDefine.EditorConfigPath}] not found.");
            }

            return 0;
        }
        private void SaveHuatuoSettings()
        {
            ...
            rootNode["UnityGameFramework"]["HuatuoSettings"]["DynamicAssemblyPlatforms"].MergeWith(
                new TomlInteger{Value = DynamicAssemblyPlatforms});
            
            TOML.ForceASCII = true;
            using (StreamWriter writer = File.CreateText(HuatuoDefine.EditorConfigPath))
            {
        ...
        // set build symbols basic on dynamic assembly platform setting
        internal static bool ApplyHuatuoSymbols(long dynamicAssemblyPlatforms)
        {
            var anySymbolModified = false;
            foreach (var buildTargetGroup in HuatuoDefine.BuildTargetGroupsOfUGFOption)
            {
                var modified = false;
                if ((dynamicAssemblyPlatforms & (1 << (int)buildTargetGroup)) == 0)
                {
                    foreach (var symbol in HuatuoDefine.HuatuoSymbols)
                    {
                        modified |= ScriptingDefineSymbols.RemoveScriptingDefineSymbol(buildTargetGroup, symbol);
                    }
                    if (modified) Debug.Log($"Build target group [ {buildTargetGroup} ] huatuo symbols removed");
                }
                else
                {
                    foreach (var symbol in HuatuoDefine.HuatuoSymbols)
                    {
                        modified |= ScriptingDefineSymbols.AddScriptingDefineSymbol(buildTargetGroup, symbol);
                    }
                    if (modified) Debug.Log($"Build target group [ {buildTargetGroup} ] huatuo symbols Added");
                }

                anySymbolModified |= modified;
            }
#if UNITY_2021_1_OR_NEWER
            if (anySymbolModified && Directory.Exists("Library/Bee/artifacts"))
            {
                Debug.Log("Clear bee compilation cache.");
                Directory.Delete("Library/Bee/artifacts", true);
            }
#endif
            return anySymbolModified;
        }
        ...
        public bool IsDynamicAssemblyTargetSelected(BuildTargetGroup targetGroup)
        {
            return (DynamicAssemblyPlatforms & (1 << (int)targetGroup)) != 0;
        }

        public void SelectDynamicAssemblyTarget(BuildTargetGroup targetGroup, bool selected)
        {
            if (selected)
            {
                DynamicAssemblyPlatforms |= 1 << (int)targetGroup;
            }
            else
            {
                DynamicAssemblyPlatforms &= ~(1 << (int)targetGroup);
        ...
        /// <summary>
        /// Build resources for selected plaforms
        /// </summary>
        /// <returns>build time in milliseconds, -1 if failed.</returns>
        public long BuildResources()
        {
            ...
            m_BuildReport.Initialize(BuildReportPath, ProductName, CompanyName, GameIdentifier, GameFrameworkVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                Platforms, DynamicAssemblyPlatforms, AssetBundleCompression, CompressionHelperTypeName, AdditionalCompressionSelected, ForceRebuildAssetBundleSelected, BuildEventHandlerTypeName, OutputDirectory, buildAssetBundleOptions, m_ResourceDatas);
            ...
            try
            {
                ...
                if (m_BuildEventHandler != null)
                {
                    ...
                    m_BuildEventHandler.OnPreprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, GameFrameworkVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                        Platforms, DynamicAssemblyPlatforms, AssetBundleCompression, CompressionHelperTypeName, AdditionalCompressionSelected, ForceRebuildAssetBundleSelected, BuildEventHandlerTypeName, OutputDirectory, buildAssetBundleOptions,
                ...
                var buildTimeInMilliseconds = m_Timer.ElapsedMilliseconds;
                m_BuildReport.LogInfo($"Build resources for selected platforms complete in {Math.Round(buildTimeInMilliseconds * 0.001f, 2)} seconds.");
                m_BuildReport.SaveReport();

                return buildTimeInMilliseconds;
            }
        ...
        private bool BuildResources(Platform platform, AssetBundleBuild[] assetBundleBuildDatas, BuildAssetBundleOptions buildAssetBundleOptions, ResourceData[] assetBundleResourceDatas, ResourceData[] binaryResourceDatas)
        {
            ...
            var startTime = m_Timer.ElapsedMilliseconds;
            ...
            m_BuildReport.LogInfo($"Build resources for '{{0}}' success in {Math.Round((m_Timer.ElapsedMilliseconds - startTime) * 0.001f, 2)} seconds.", platformName);
            return true;
        }
